...
if(doctors.length > 0 && speciality){
  ...
  return (
      ...
      <div className='w-full grid grid-cols-auto gap-4 pt-5 gap-y-6 px-3 sm:px-0'>
        {
            reldoc.slice(0,5).map((item,index)=>{
                ...
                <div onClick={()=>{navigate(`/appointment/${item._id}`); scrollTo(0,0)} }className=' border border-green-300 rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-10px] transition-all duration-500'key={index}>
                    <img src={item.image} className='bg-green-100 '/>
                    <div className='p-4'>
      ...
      <button onClick={()=>{navigate('/doctors');scrollTo(0,0)}} className='bg-green-900 text-green-200 px-12 py-3 rounded-full mt-10'>Show More</button>
    </div>
